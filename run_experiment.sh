...
SESSION="cpu_imu_exp"
ROSBAG="${ROSBAG:-1}"
GLOBAL_PARAMS="$REPO/src/main/launch/params/global.yaml"
...
run_pane "$SESSION:sensing.1" \
    "ros2 run hardware imu --ros-args --params-file '$GLOBAL_PARAMS'"
sleep 1
...
run_pane "$SESSION:thrust.0" \
    "ros2 run control thrust_generator --ros-args --params-file '$GLOBAL_PARAMS' --log-level warn"
run_pane "$SESSION:thrust.1" \
    "ros2 run hardware thrusters --ros-args --params-file '$GLOBAL_PARAMS' --log-level warn"
sleep 1
run_pane "$SESSION:thrust.2" \
    "ros2 run hardware arduino --ros-args --params-file '$GLOBAL_PARAMS'"
