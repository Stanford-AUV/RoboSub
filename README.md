...
├── teleop_remote.sh               # Sub-side joystick teleop: nats-server + manual/joystick node
...
**`manual/joystick`** uses **`nats://localhost:4222`**, subject **`joystick`**. On the sub run **`hardware.py`** (for **`thrust_generator`** + real thrusters), then **`./teleop_remote.sh`** (starts `nats-server` + the joystick node). On the pilot laptop run the standalone **`robosub_local/run.sh --host <sub-tether-ip>`**. Do **not** run joystick/keyboard together with the **`main.py`** autonomy stack on the same bring-up if you want only controller wrenches on **`/wrench`**.
