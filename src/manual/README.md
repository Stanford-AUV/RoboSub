...
**On the sub (Orin):**
1. Bring up hardware (real thrusters + depth + arduino/light): `ros2 launch main hardware.py`.
2. Start teleop: `./teleop_remote.sh` (from repo root). It starts `nats-server`
   (binds `0.0.0.0:4222`) and the `manual/joystick` node. Tune push with
   `./teleop_remote.sh -p wrench_scale:=1.5` (default `2.0`).
