...
DATA_TYPES = {"position", "orientation", "velocity", "angular", "accel"}
METADATA_FIELDS = {"sensor_pos_in_base", "R_sensor_to_base"} | {
...
class GenericSensor(Node):
    ...
    (e.g. orientation_covariance) are accessible via get_covariance(). Metadata fields
    ...
    Subclasses are responsible for:
      ...
      - Populating self._pubbers with their data-type keyed publishers.
    ...
    def _create_publishers(self):
        if self.is_active("position"):
            self.sensor_publishers["position"] = self.create_publisher(
                PoseWithCovarianceStamped, "/position", 10
            )
        if self.is_active("orientation"):
            self.sensor_publishers["orientation"] = self.create_publisher(
                PoseWithCovarianceStamped, "/orientation", 10
            )
        if self.is_active("velocity"):
            self.sensor_publishers["velocity"] = self.create_publisher(
