...
class IMU(GenericSensor):
    ...
    def _build_orientation_cov(self):
        """6x6 pose covariance: large values for position, yaml values for orientation."""
        cov = np.zeros(36)
        rot_axes = self.get_axes("orientation")
        rot_cov = self.get_covariance("orientation")
    ...
    def publish_sensor_data(self):
        ...
        if self.is_active("orientation"):
            ...
            rot_msg.pose.covariance = self._build_orientation_cov()
            self.sensor_publishers["orientation"].publish(rot_msg)
